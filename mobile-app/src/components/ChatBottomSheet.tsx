  ...
  return (
    <View style={styles.voicePlayer}>
      <TouchableOpacity
        ...
        style={[styles.playBtn, isUser ? styles.userPlayBtn : styles.aiPlayBtn]}
      >
        <MaterialIcons
          name={isPlaying ? "pause" : "play-arrow"}
          size={20}
          color={isUser ? "#00595c" : "#fff"}
        />
      </TouchableOpacity>
      <View
        style={[
          styles.waveformContainer,
          isUser ? styles.userWaveformBg : styles.aiWaveformBg,
        ]}
      >
        <View
          style={[
      ...
      <Text
        style={[
          ...
          isUser ? styles.userBubbleTime : styles.aiBubbleTime,
        ]}
      >
...
export default function ChatBottomSheet({
  ...
  const [bookingProposal, setBookingProposal] =
    useState<BookingProposal | null>(null);
  ...
  const [selectedWhyHimProvider, setSelectedWhyHimProvider] =
    ...
    try {
      ...
      if (status === "granted") {
        ...
        if (reverseGeocode && reverseGeocode.length > 0) {
          const { city, district, region, subregion, name, street } =
            reverseGeocode[0];
  ...
  async function fetchReply(
    ...
    try {
      ...
      if (providerMode) {
        ...
        if (
          response.phase === "confirming_booking" &&
  ...
  function handleDeclineBooking(msgId: string) {
    ...
    const updatedHistory = [
      ...messages.map((m) =>
        m.id === msgId ? { ...m, proposalStatus: "declined" as const } : m,
      ),
      userMsg,
    ];
    setMessages(updatedHistory);
  ...
  function handleCancelBooking(msgId: string, booking: CancelBookingItem) {
    const text = `YES, I want to cancel my ${booking.service_type} booking (ID: ${booking.booking_id}) with ${booking.provider_name} on ${booking.date} at ${booking.slot}.`;
    const userMsg: Message = {
      id: `u${Date.now()}`,
      role: "user",
      text,
      ...
      <View style={styles.backdrop}>
        ...
        >
          ...
          <View style={styles.header}>
            <View style={styles.headerLeft}>
              <View
                style={[
                  styles.aiAvatar,
                  providerMode && { backgroundColor: "#005c3e" },
                ]}
              >
                <MaterialIcons
                  name={providerMode ? "person" : "smart-toy"}
                  size={20}
                  color="#fff"
                />
              </View>
              <View>
                <Text style={styles.headerTitle}>
                  {providerMode ? providerName : "Karigar Assistant"}
                </Text>
                <Text style={styles.headerSub}>
                  {providerMode ? "Service Provider (Active)" : "Powered by AI"}
                </Text>
              </View>
            </View>
            <TouchableOpacity
          ...
          >
            ...
            <View style={styles.systemChip}>
              <Text style={styles.systemChipText}>
                Chat started · {formatTime(new Date())}
              </Text>
            </View>
                    ...
                    {msg.role === "assistant" && (
                      ...
                      >
                        <MaterialIcons
                          name={providerMode ? "person" : "smart-toy"}
                          size={14}
                          color={providerMode ? "#004B87" : "#00595c"}
                        />
                      </View>
                    )}
                    <View
                  ...
                  {msg.selectable &&
                    ...
                    msg.providers.length > 0 && (
                      <View style={styles.providersContainer}>
                        <Text style={styles.providersHeader}>
                          Available Providers
                        </Text>
                        <ScrollView
                          horizontal
                        ...
                        >
                          {msg.providers.map((p) => (
                            ...
                            >
                                ...
                                onPress={() => {
                                  router.push({
                                    ...
                                    params: {
                                      ...
                                      price: p.hourly_rate
                                        ? `Rs ${p.hourly_rate} Base Fee`
                                        : "Rs 1,200 Base Fee",
                                      smart_match: p.smart_match
                                        ? JSON.stringify(p.smart_match)
                                        : undefined,
                                    },
                                  });
                                }}
                                ...
                                {p.smart_match?.is_top_pick && (
                                  <View style={styles.topPickBadgeInCard}>
                                    <MaterialIcons
                                      ...
                                      color="#92400e"
                                    />
                                    <Text style={styles.topPickBadgeTextInCard}>
                                      AI TOP PICK
                                    </Text>
                                  </View>
                                )}
                              ...
                              <Text style={styles.providerSlotsTitle}>
                                Available Slots:
                              </Text>
                              <View style={styles.slotsContainer}>
                                {(Array.isArray(p.available_slots)
                                  ...
                                  <TouchableOpacity
                                    ...
                                    style={[
                                      styles.slotBtn,
                                      isOldMessage && styles.disabledSlotBtn,
                                    ]}
                                    disabled={isOldMessage}
                                    onPress={() => {
                    ...
                    <View style={styles.bookingCard}>
                      ...
                      <View style={styles.bookingRow}>
                        ...
                        <Text style={styles.bookingValue}>
                          {msg.bookingProposal.service}
                        </Text>
                      </View>
                      <TouchableOpacity
                        style={styles.bookingRow}
                        onPress={() => {
                          const matchingProvider = msg.providers?.find(
                            (p) => p.id === msg.bookingProposal!.provider_id,
                          );
                          router.push({
                            pathname: "/provider-profile",
                            params: {
                              id:
                                msg.bookingProposal!.provider_id?.toString() ||
                                "unknown",
                              name: msg.bookingProposal!.provider,
                              service: msg.bookingProposal!.service,
                              price: msg.bookingProposal!.price,
                              smart_match: matchingProvider?.smart_match
                                ? JSON.stringify(matchingProvider.smart_match)
                                : undefined,
                            },
                          });
                        }}
                      ...
                      <View style={styles.bookingRow}>
                        ...
                        <Text style={styles.bookingValue}>
                          {msg.bookingProposal.time}
                        </Text>
                      </View>
                      <View
                        style={[styles.bookingRow, { borderBottomWidth: 0 }]}
                      >
                      ...
                      >
                        ...
                        <TouchableOpacity
                          style={[
                            styles.confirmBtn,
                            isOldMessage && styles.disabledConfirmBtn,
                          ]}
                          disabled={isOldMessage}
                          onPress={() =>
                            handleConfirmBooking(msg.id, msg.bookingProposal!)
                          }
                        >
                          <MaterialIcons
                            name="check"
                            size={16}
                            color="#fff"
                            style={isOldMessage && { opacity: 0.5 }}
                          />
                          <Text
                            style={[
                              styles.confirmBtnText,
                              isOldMessage && styles.disabledConfirmBtnText,
                            ]}
                          >
                            Confirm Booking
                          </Text>
                  ...
                  {msg.cancelBookings && msg.cancelBookings.length > 0 && (
                    <View style={{ gap: 10, marginTop: 4 }}>
                      {msg.cancelBookings.map((cb) => {
                        const isCancelled = msg.cancelledBookingIds?.includes(
                          cb.booking_id,
                        );
                        return (
                          <View key={cb.booking_id} style={styles.cancelCard}>
                            <View style={styles.cancelCardTop}>
                              <View style={styles.cancelCardIcon}>
                                <MaterialIcons
                                  name="event-busy"
                                  size={18}
                                  color="#c62828"
                              ...
                              <View style={{ flex: 1 }}>
                                <Text style={styles.cancelCardService}>
                                  {cb.service_type}
                                </Text>
                                <Text style={styles.cancelCardProvider}>
                                  {cb.provider_name}
                                </Text>
                              </View>
                              <Text style={styles.cancelCardPrice}>
                                Rs. {cb.price}
                              </Text>
                            </View>
                            <View style={styles.cancelCardDivider} />
                            <View style={styles.cancelCardMeta}>
                              <View style={styles.cancelCardMetaItem}>
                                <MaterialIcons
                                  name="calendar-today"
                                  size={13}
                                  color="#8b6b6b"
                                />
                                <Text style={styles.cancelCardMetaText}>
                                  {cb.date}
                                </Text>
                              </View>
                              <View style={styles.cancelCardMetaItem}>
                              ...
                              <Text style={styles.cancelCardId}>
                                #{cb.booking_id}
                              </Text>
                            </View>
                            {isCancelled ? (
                              ...
                              <TouchableOpacity
                                style={[
                                  styles.cancelCardBtn,
                                  isOldMessage && { opacity: 0.5 },
                                ]}
                                disabled={isOldMessage}
                                onPress={() => handleCancelBooking(msg.id, cb)}
                              >
                                <Text style={styles.cancelCardBtnText}>
                                  Cancel This Booking
                                </Text>
                              </TouchableOpacity>
                            )}
              ...
              <View style={[styles.bubble, styles.aiBubble]}>
                ...
                <View
                  style={[
                    ...
                    providerMode && { backgroundColor: "#f0f7fc" },
                  ]}
                >
                  <ActivityIndicator
                    size="small"
                  ...
                  <Text
                    style={[
                      styles.typingText,
                      providerMode && { color: "#004B87" },
                    ]}
                  >
                    {providerMode
                      ? `${providerName} is typing...`
                      : "Karigar AI is thinking..."}
                  </Text>
                </View>
              </View>
            )}
              ...
              <View style={styles.inputRow}>
                ...
                {inputText.trim().length > 0 ? (
                  <TouchableOpacity
                    style={[
                      styles.chatSendBtn,
                      isTyping && styles.chatSendBtnDisabled,
                    ]}
                    onPress={handleSend}
                    disabled={isTyping}
                  >
            ...
            <View style={styles.confirmOverlay}>
              <View style={styles.confirmCard}>
                ...
                <Text style={styles.confirmBody}>
                  ...
                  to navigate to your bookings?
                </Text>
                <View style={styles.confirmActions}>
                  <TouchableOpacity
                    style={styles.confirmKeepBtn}
                    onPress={() => setShowNavigateConfirm(false)}
                  >
          ...
          {showCloseConfirm && (
            <View style={styles.confirmOverlay}>
              <View style={styles.confirmCard}>
                ...
                <Text style={styles.confirmBody}>
                  Your conversation will be lost. Are you sure you want to
                  leave?
                </Text>
                <View style={styles.confirmActions}>
                  <TouchableOpacity
          ...
          {whyHimModalVisible && selectedWhyHimProvider && (
            <View style={styles.confirmOverlay}>
              <View
                style={[styles.confirmCard, { width: "90%", maxHeight: "80%" }]}
              >
                <View
                  style={{
                    flexDirection: "row",
                ...
                >
                  ...
                  >
                    ...
                    >
                      Antigravity AI Match Analysis
                    </Text>
                  </View>
                  <TouchableOpacity
                    onPress={() => {
                      setWhyHimModalVisible(false);
                      setSelectedWhyHimProvider(null);
                    }}
                    style={{ padding: 4 }}
                  >
                ...
                <ScrollView
                  ...
                  showsVerticalScrollIndicator={false}
                >
                  {selectedWhyHimProvider.smart_match?.match_reasons?.map(
                    (reason: any, idx: number) => {
                      let iconName: any = "done";
                      if (reason.factor === "proximity")
...
const styles = StyleSheet.create({
  ...
  cancelCardTop: { flexDirection: "row", alignItems: "center", gap: 10 },
  cancelCardIcon: {
    width: 36,
    height: 36,
    borderRadius: 10,
  ...
  cancelCardMeta: { flexDirection: "row", alignItems: "center", gap: 14 },
  cancelCardMetaItem: { flexDirection: "row", alignItems: "center", gap: 4 },
  cancelCardMetaText: {
    fontFamily: "Inter_400Regular",
    fontSize: 12,
    color: "#8b6b6b",
  ...
  topPickBadgeTextInCard: {
    fontFamily: "PlusJakartaSans_600SemiBold",
    fontSize: 8,
    color: "#92400e",
  },
  providerAvatarInCard: {
    width: 32,
    height: 32,
    borderRadius: 16,
    marginRight: 8,
